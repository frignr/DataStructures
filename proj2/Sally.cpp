   ...
   istrm(input_stream)  // use member initializer to bind reference
{
   ...
   symtab["<"]    =  SymTabEntry(KEYWORD,0, &less) ;
   symtab["<="]   =  SymTabEntry(KEYWORD,0, &lessOrEqual) ;
   symtab["=="]   =  SymTabEntry(KEYWORD,0, &equal) ;
   symtab["!="]   =  SymTabEntry(KEYWORD,0, &notEqual) ;
   symtab[">="]   =  SymTabEntry(KEYWORD,0, &greaterOrEqual) ;
   symtab[">"]    =  SymTabEntry(KEYWORD,0, &greater) ;
...
void Sally::doDUMP(Sally *Sptr) {
  ...
  stack<Token> dump = Sptr->params;
  Token tk;
  while(!dump.empty()){
    tk = dump.top();
    cout << tk.m_kind << "," << tk.m_value << "," << tk.m_text << endl;
    dump.pop();
  }
} 
...
void Sally::doDup(Sally *Sptr) {
  ...
  p = Sptr->params.top();
  Sptr->params.push(Token(p.m_kind, p.m_value, p.m_text));
}
...
void Sally::doSet(Sally *Sptr){
  ...
  if(it == Sptr->symtab.end()){
    Sptr->symtab[var.m_text] = SymTabEntry(VARIABLE, val.m_value, NULL);
  }
  else{
    cerr << "Variable name already taken"<< endl;
  }
}
...
void Sally::doAt(Sally *Sptr){
  ...
  it = Sptr->symtab.find(var.m_text);
  if(it == Sptr->symtab.end()){
    cerr<< "No such variable exists." << endl;
  }  
  else{
...
void Sally::lessOrEqual(Sally *Sptr){
  Token left, right;
  if (Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for <=");
  }
...
void Sally::notEqual(Sally *Sptr){
  Token left, right;
  if (Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for !=");
  }
...
void Sally::greaterOrEqual(Sally *Sptr){
  Token left, right;
  if (Sptr->params.size() < 2){
    throw out_of_range("Need two parameters for >=");
  }
...
void Sally::doIfThen(Sally *Sptr){
  ...
  Token boolean;
  boolean = Sptr->params.top();
  bool keepGoing = true;
  Token tk;
  int ifCounter = 0;
  if(boolean.m_value == 0){
    while (keepGoing){
      tk = Sptr->nextToken();
      if(tk.m_text == "IF"){
	ifCounter ++;
...
void Sally::doElse(Sally *Sptr){
  if(Sptr->params.size() <1){
    throw out_of_range("IFTHEN condition used up. Programming error");
  }
  Token boolean;
  boolean = Sptr->params.top();
  Sptr->params.pop();
...
void Sally::doDo(Sally *Sptr){
  list<Token> doLoop;
  int doCounter = 0; //Counts any additional dos that might occur
  Token tk;
  bool keepGoing = true;
  list<Token>::iterator it;
  while (keepGoing){
    tk = Sptr-> nextToken();
    doLoop.push_back( Token(tk.m_kind, tk.m_value, tk.m_text) );
    if(tk.m_text =="DO") {
      doCounter++;
    }
    if(tk.m_text == "UNTIL"){
  ...
  for(list<Token>::reverse_iterator it = doLoop.rbegin(); it != doLoop.rend(); it++){
    Sptr->tkBuffer.push_front( Token(it->m_kind, it->m_value, it->m_text) );
  }
  Sptr->doLists.push(doLoop);
}

void Sally::doUntil(Sally *Sptr){
  if(Sptr->params.size() < 1){
    throw out_of_range("Need at least one parameter for UNTIL");
  }
  Token boolean = Sptr->params.top();
  Sptr->params.pop();
  if (boolean.m_value != 0){
    Sptr->doLists.pop();
  }else{
    list<Token> doLoop = Sptr->doLists.top();
    for(list<Token>::reverse_iterator it = doLoop.rbegin(); it!= doLoop.rend(); it++){
      Sptr->tkBuffer.push_front( Token(it->m_kind, it->m_value, it->m_text) );
    }
  }
